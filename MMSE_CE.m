...
H_ = LS_CE(Y, Xp, pilot_loc, Nfft, Nps, 'spline');  % \tilde{H}
H = fft(h, Nfft);
RHH_ = transpose(H)*conj(H_);
RH_H_ = transpose(H_)*conj(H_);
noise = (randn(1,Nfft) + 1j*randn(1, Nfft))/sqrt(snr/2);
H_MMSE = transpose(RHH_ / RH_H_ * transpose(H_)) + noise/2;